...
public class UserThirdWebController {
    ...
    @ApiOperation("江西银行网络交易资金账户服务三方协议")
    @GetMapping("/thirdAccount/protocol/json")
    public ResponseEntity<VoHtmlResp> thirdAccountProtocolJson(@ApiIgnore @RequestAttribute(SecurityContants.USERID_KEY) Long userId, Model model) {
        VoHtmlResp voHtmlResp = VoBaseResp.ok("查询成功!", VoHtmlResp.class);
        voHtmlResp.setHtml(userThirdBiz.thirdAccountProtocolJson(userId));
