...
public class VoCreateLend {
    ...
    @ApiModelProperty(name = "money", value = "借款金额 min:1000|max:20000000", dataType = "int", required = true)
    ...
    @ApiModelProperty(name = "lowest", value = "最低金额(分) min:1000|max:借款金额", dataType = "int", required = true)
    @NotNull(message = "最低金额不能为空!")
    @Min(value = LendContants.MIN_LOWEST, message = "最低金额不能低于" + LendContants.MIN_LOWEST/100 + "元!")
    private Integer lowest; //最低金额
