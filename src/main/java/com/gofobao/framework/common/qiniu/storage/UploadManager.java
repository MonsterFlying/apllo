...
/**
 * 七牛文件上传管理器，通过该类上传文件时，会自动根据定义的{@link com.qiniu.storage.Configuration#putThreshold}
 * 来判断是采用表单上传还是分片上传的方法，超过了定义的{@link com.qiniu.storage.Configuration#putThreshold}就会采用
 * 分片上传的方法，可以在构造该类对象的时候，通过{@link com.qiniu.storage.Configuration}类来自定义这个值。
