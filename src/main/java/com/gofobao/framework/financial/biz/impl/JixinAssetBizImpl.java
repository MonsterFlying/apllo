...
public class JixinAssetBizImpl implements JixinAssetBiz {
    ...
    @Override
    public boolean record(String cardnbr,String crflag,String amount) {    // 判断即信资金记录是否存在
        JixinAsset jixinAsset = jixinAssetService.findTopByAccountId(cardnbr);
        ...
        if (ObjectUtils.isEmpty(jixinAsset)) { // 发现没有就创建
            ...
            if (ObjectUtils.isEmpty(userThirdAccount)) {
                log.error(String.format("即信金额统计, 根据电子账号信息, 查询用户信息为空, 数据[%s]", cardnbr));
                return false;
            }
        ...
        jixinAssetService.save(jixinAsset);
        return true;
    }
}
